...
class ShortestPath(torch.autograd.Function):
    ...
    def forward(ctx, weights, lambda_val, neighbourhood_fn="8-grid"):
        ...
        ctx.save_for_backward(weights, torch.from_numpy(suggested_tours).float().to(weights.device))
    ...
    def backward(ctx, grad_output):
        ...
        weights_np = weights.detach().cpu().numpy()
        weights_prime = np.maximum(weights_np + lambda_val * grad_output_numpy, 0.0)
        better_paths = np.asarray(maybe_parallelize(solver, arg_list=list(weights_prime)))
        gradient = -(suggested_tours.cpu().numpy() - better_paths) / lambda_val
